...
| Файл | За что отвечает |
|---|---|
| `translator.py` | Ядро перевода: группировка блоков в сегменты (`build_segments`, гигантские абзацы режутся по предложениям), параллельный перевод партиями, цикл исправлений с фидбеком валидаторов, пауза, чекпойнты. |

...
| Файл | За что отвечает |
|---|---|
| `translator.py` | Ядро перевода: группировка блоков в сегменты (`build_segments`, гигантские абзацы режутся по предложениям), дедупликация одинаковых сегментов (кэш прогона), параллельный перевод партиями через один пул потоков, цикл исправлений с фидбеком валидаторов (ранний выход при повторе ответа), доперевод остатков на языке оригинала (`retranslate_residual`), пауза, чекпойнты. |

...
| `html_tables.py` | HTML-таблицы парсеров (MinerU, OCR) → Markdown-таблицы: иначе блочный HTML не переводится, а рваные `</td><td>` ломают модель. Целые `<table>` и голые `<tr>`-последовательности; неразбираемое не трогается. |

### Перевод (`pdftransl/translation/`)

| Файл | За что отвечает |
|---|---|
| `translator.py` | Ядро перевода: группировка блоков в сегменты (`build_segments`, гигантские абзацы режутся по предложениям), дедупликация одинаковых сегментов (кэш прогона), параллельный перевод партиями через один пул потоков, цикл исправлений с фидбеком валидаторов (ранний выход при повторе ответа), спасение переводом без маскировки при упорной потере плейсхолдеров (`unmasked_rescue`), доперевод остатков на языке оригинала (`retranslate_residual`), пауза, чекпойнты. |
...
| Файл | За что отвечает |
|---|---|
| `store.py` | TM в SQLite: точные совпадения (human-правки приоритетнее), косинусный поиск похожих по кэшированному снимку (numpy-матрица, инвалидация записью/TTL), доменный фильтр, экспорт датасета для дообучения. |
...
| `openai_compat.py` | Клиент любого OpenAI-совместимого `/chat/completions`: OpenAI, OpenRouter, DeepSeek, Ollama, vLLM, LM Studio…; keep-alive-сессия с пулом соединений, раздельный connect/read-таймаут, ретраи с джиттером, Retry-After, ретрай битого/пустого ответа, DEBUG-телеметрия. |
| `anthropic_client.py` | Нативный клиент Anthropic Messages API (system отдельно, свой формат картинок); та же обвязка стабильности, что у OpenAI-клиента. |
| `fallback.py` | `FallbackClient` — «липкая» цепочка провайдеров: упал первый → следующий; рабочий запоминается, основной пробуется снова раз в 5 минут. |

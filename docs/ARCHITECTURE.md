...
2. **split + references** — `parsing/splitter.py` разбивает Markdown на
   ...
   цитируемость. **Любой** следующий заголовок завершает секцию (а
   заголовок-«литература» снова её открывает) — раньше секцию завершал
   только узкий список заголовков (Appendix и т.п.), из-за чего
   Conclusion/Nomenclature после библиографии оставались непереведёнными.
...
4. **mask + segment** — `masking.py` заменяет формулы/код/ссылки на
   ...
   `chunk_char_budget`. Абзац, который **один** больше бюджета
   (типичный источник — страница битого PDF, извлёкшаяся простынёй без
   пустых строк), режется по границам предложений — иначе один
   гигантский запрос переполняет контекст/`max_tokens` модели и
   возвращается обрезанным. Таблицы и заголовки не режутся никогда
   (частичная таблица ломает и markdown, и валидатор строк); одиночное
   предложение длиннее бюджета остаётся целым (разрез посреди
   предложения вредит переводу сильнее длинного запроса).
...
6. **translate** — `translation/translator.py`: сегменты независимы и
   ...
   **партиями** по `translate_batch_size` сегментов (по умолчанию 40; 0 —
   ...
   успешного парсинга при затупах LLM. Тот же откат срабатывает, если
   после цикла исправлений в сегменте остались ошибки
   `placeholder_missing`/`placeholder_unknown` — перевод, потерявший
   плейсхолдеры, потерял формулы/цитаты/ссылки, и отдавать его хуже, чем
   оставить сегмент на языке оригинала (в QA-отчёте он помечен, в
   вычитке — правится руками; `unmask` при этом разворачивает и
   вложенные плейсхолдеры до неподвижной точки).
...
### Оценка времени до результата (ETA)

`estimate_eta_seconds(elapsed, progress)` — та же линейная экстраполяция,
что у pip/tqdm: «сколько ушло на текущий процент — столько же (в той же
пропорции) уйдёт на оставшийся». Это приближение, а не обещание — темп
парсинга и перевода не постоянен — но оценка самокорректируется на
каждом тике по мере того, как накапливаются реальные данные. `None`,
пока прогресс меньше 3% (иначе знаменатель слишком мал и оценка
бессмысленна) и после завершения.

- `TranslationJob.started_at` — момент, когда воркер реально начал
  выполнение (не постановка в очередь и не создание задачи), обнуляется
  при каждом (пере)диспатче, включая возобновление после паузы — иначе
  время ожидания в очереди/на паузе исказило бы оценку до абсурдных
  значений сразу после возобновления.
- `TranslationJob.eta_seconds()` пересчитывается на каждый запрос
  `as_dict()` и на каждый тик SSE-стрима задачи — а поскольку оно меняется
  практически каждую секунду, пока задача выполняется, стрим `/jobs/<id>/events/`
  теперь естественно шлёт обновление раз в секунду (живой обратный отсчёт),
  а не только при смене стадии.
- Веб: `formatEta()` (`frontend/src/format.js`) форматирует секунды в
  «~N мин» / «~H ч M мин» / «меньше минуты»; показывается и в
  `JobDetail.jsx`, и в строке задачи в `JobList.jsx`.

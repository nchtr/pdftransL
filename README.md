...
формулы (в LaTeX). Работает и локально: `--provider ollama --vision-model
qwen2.5-vl`, или явно `--backend vlm_ocr`. Если vision-модели нет, задача
завершится с предупреждением в отчёте (а не молча пустым переводом).
